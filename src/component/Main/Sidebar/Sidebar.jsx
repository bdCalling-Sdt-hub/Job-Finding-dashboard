...
import logo from "/Images/Auth/logo2.png";
...
import { HiLightBulb, HiOutlineDocumentText, HiOutlineUsers } from "react-icons/hi";
...
const sidebarItems = [
  ...
  },
  {
    path: "/companies",
    name: "Companies",
    icon: <BiBuildingHouse className="size-6" />,
  },
  ...
  },
  {
    path: "/job-listing",
    name: "Job Listing",
    icon: <PiBagLight className="size-6" />,
  },
  {
...
const Sidebar = ({ isSidebarOpen, toggleSidebar }) => {
  ...
  return (
    <div>
      {/* Desktop Sidebar */}
      <div className="hidden overflow-y-auto md:block w-full md:w-[200px] lg:w-[250px] xl:w-[280px] h-full bg-[#e0f6d7] fixed  border-r-2 border-[#9a4ed4]">
        <Link to={"/"} className="flex flex-col justify-center items-center pt-5 gap-2  mb-10 text-black">
          <img src={logo} alt="logo" className="w-2/3 mx-auto my-10 " />
        </Link>
        <ul className="flex flex-col gap-5">
          {sidebarItems.map((item) => (
            <NavLink
              ...
              to={item.path}
              className={({ isActive }) =>
                `w-[80%] mx-auto px-5 py-2 flex justify-start items-center gap-3 text-[#6d6d6d] ${isActive ? "bg-white !text-[#553283] border-l-4 border-[#553283]" : ""
                }`
              }
            >
              {item?.icon}
      ...
      {/* Mobile Sidebar */}
      <div
        className={`fixed top-0 overflow-y-auto left-0 z-40 w-64 h-full bg-[#e0f6d7] shadow-lg transform ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"
        ...
        <ul className="flex flex-col gap-3 mt-10">
          {sidebarItems.map((item) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `w-[70%] mx-auto px-5 py-2 flex items-center gap-3  ${isActive ? "bg-[#553283] text-white" : "text-[#6d6d6d]"
                }`
              }
            >
              {item?.icon}
